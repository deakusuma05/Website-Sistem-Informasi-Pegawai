...
@section('header-subtitle', 'Modify employee record information and tenure details')
    ...
    <div class="flex items-center justify-between">
        <nav class="flex text-xs font-medium text-slate-500" aria-label="Breadcrumb">
            <ol class="inline-flex items-center space-x-1.5">
                ...
                <li>
                    <a href="{{ route('employees.show', $employee) }}" class="hover:text-teal-600 transition-colors">{{ $employee->name }}</a>
                </li>
                <li>
                    <span class="text-slate-400">/</span>
                </li>
                <li class="text-teal-600 font-semibold" aria-current="page">Edit</li>
            </ol>
        </nav>
    ...
    <div class="soft-card rounded-2xl overflow-hidden">
        ...
        <div class="p-6 bg-gradient-to-r from-[#0B132B] to-[#1C2541] text-white flex items-center justify-between">
            <div class="flex items-center gap-3.5">
                <div class="w-12 h-12 rounded-xl bg-cyan-500/20 border border-cyan-400/30 flex items-center justify-center text-cyan-300">
                    <svg class="w-6 h-6" fill="none" viewBox="0 0 24 24" stroke="currentColor" stroke-width="2">
                        <path stroke-linecap="round" stroke-linejoin="round" d="M11 5H6a2 2 0 00-2 2v11a2 2 0 002 2h11a2 2 0 002-2v-5m-1.414-9.414a2 2 0 112.828 2.828L11.828 15H9v-2.828l8.586-8.586z" />
                    </svg>
                </div>
                <div>
                    <h2 class="text-base font-bold text-white tracking-tight">Edit Employee: {{ $employee->name }}</h2>
                    <p class="text-xs text-slate-300">Record ID #{{ $employee->id }} &bull; Registered on {{ $employee->created_at->format('M d, Y') }}</p>
                </div>
            </div>
            <span class="text-xs font-medium px-3 py-1 rounded-full bg-cyan-400/20 text-cyan-300 border border-cyan-400/30 hidden sm:inline-block">
                Edit Mode
            </span>
        </div>

        <!-- Form Body -->
        <form action="{{ route('employees.update', $employee) }}" method="POST" class="p-6 sm:p-8 space-y-6">
            @csrf
            @method('PUT')
            ...
            @if ($errors->any())
                <div class="p-4 rounded-2xl bg-rose-50 border border-rose-200 text-rose-800 flex items-start gap-3">
                    ...
                    <div class="flex-1 text-xs sm:text-sm">
                        <p class="font-bold text-rose-900">Update Failed: {{ $errors->count() }} {{ Str::plural('error', $errors->count()) }} found</p>
            ...
            <div class="grid grid-cols-1 sm:grid-cols-2 gap-6">
                ...
                <div>
                    ...
                    <input type="number" 
                           ...
                           id="work_duration" 
                           min="0" 
                           value="{{ old('work_duration', $employee->work_duration) }}"
                           placeholder="e.g. 3" 
